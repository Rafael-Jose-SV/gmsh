...
class GMSHGenerator:
    ...
    def __add_points(self):
        ...
        for point in points:
          self.__gmsh.model.geo.addPoint(point.x, point.y, point.z, self.__mesh_size, point.tag)
    ...
    def __add_plane_surfaces(self):
        ...
        for plane_surface in plane_surfaces:
          self.__gmsh.model.geo.addPlaneSurface(plane_surface.plane_surface, plane_surface.tag)

    def __add_physical_groups(self):
        physical_groups = self.__gmsh_data.physical_groups

        for physical_group in physical_groups:
          self.__gmsh.model.geo.addPhysicalGroup(physical_group.dim, physical_group.tags, physical_group.tag)
